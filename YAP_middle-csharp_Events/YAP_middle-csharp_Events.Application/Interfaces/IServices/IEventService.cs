...
namespace YAP_middle_csharp_Events.Application.Interfaces.IServices
{
    ...
    {
        Task<PaginatedResult<EventUpdateRequest>> FindAllAsync(string? title = null, DateTime? from = null, DateTime? to = null, int page = 1, int pageSize = 10, CancellationToken cancellationToken = default);
        Task<IReadOnlyList<EventUpdateRequest>> FindTop10EventsAsync(CancellationToken cancellationToken = default);
        Task<EventUpdateRequest> FindByIdAsync(Guid id, CancellationToken cancellationToken = default);
